import{o as T}from"./assets/quote-section-9eaabcdc.js";import{a as F,i as H}from"./assets/vendor-8cce9181.js";const w=document.querySelector(".exercises-title-span"),f=document.querySelector(".exercises-list"),b=document.querySelector(".exercises-list-pages");b.addEventListener("click",I);f.addEventListener("click",V);const p=document.querySelector(".exercises-form");p.addEventListener("submit",j);p.addEventListener("input",N);const S=document.querySelector(".exercises-button-list"),L=document.querySelector(".form-reset-text-button");L.addEventListener("click",t=>{p.reset(),L.classList.add("reset-button-close")});let g=1,k=8,v="Muscles",d="",h="",$=0;const O=document.querySelectorAll(".exercises-button");[...O].map((t,e)=>{e===0&&t.classList.add("exercises-button-active"),t.addEventListener("click",B)});async function E(t,e,n,i,a,l){let s={};a&&e=="Body parts"&&(e="Body part"),a?s[e.toLowerCase().replace(" ","")]=a:s.filter=e,s.page=n,s.limit=i,l&&(s.keyword=l),document.body.clientWidth>=768&&!a&&(s.limit=12),document.body.clientWidth<=768&&a&&(s.limit=8),document.body.clientWidth>=1440&&a&&(s.limit=9);let r=new URLSearchParams(s).toString();try{const o=await F.get(`https://energyflow.b.goit.study/api/${t}?${r}`);if($=o.data.totalPages,l&&o.data.results.length==0){A();return}return o.data}catch(o){console.log(o)}}function A(){f.innerHTML=`<div class="no-results-wrapper">
   ...
   10.6101 6.41222 10.8647L4.19983 12.4721C3.41612 13.0415 2.36164 12.2754 2.66099 11.3541L3.50604 8.75329C3.63992 8.34127 3.49326
    7.8899 3.14277 7.63525L0.930391 6.02787C0.146677 5.45846 0.549452 4.21885 1.51818 4.21885H4.25283C4.68606 4.21885 5.07001 3.93989
     5.20389 3.52786L6.04894 0.927052Z" fill="#EEA10C"/>
        ...
        </li>`,"");f.classList.add("card-row-gap-js"),f.innerHTML=i,n()}if(!t[0]._id){let i=t.reduce((a,{filter:l,imgUrl:s,name:r})=>(p.dataset.filter=l,a+=`<li class="exercises-item" data-type="${r}" style="background-image: linear-gradient(
      0deg,
      rgba(16, 16, 16, 0.7) 0%,
      rgba(16, 16, 16, 0.7) 100%
        ...
        </li>`),"");p.dataset.status="",f.classList.remove("card-row-gap-js"),f.innerHTML=i,n(t.filter)}function n(i){p.dataset.status="",b.innerHTML=R(g,e)}}function I(t){const e=t.target.dataset;if(t.target.dataset.type||e.id){if(g==t.target.dataset.number)return;if(g=t.target.dataset.number,e.id&&(g=$,e.id=="left"&&(g=1)),document.querySelector(".exercises-section").scrollIntoView({behavior:"smooth",block:"start"}),d==""){M(t.target.dataset.type);return}x(t.target.dataset.type,t.target.dataset.card,h)}}function R(t,e){t=Number.parseInt(t);let n=`<svg class="page-choice-svg" width="18" height="18" data-card="${d}" data-id="left" data-type="${v}" xmlns="http://www.w3.org/2000/svg" id="Outline" viewBox="0 0 24 24" width="512" height="512"><path d="M17.17,24a1,1,0,0,1-.71-.29L8.29,15.54a5,5,0,0,1,0-7.08L16.46.29a1,1,0,1,1,1.42,1.42L9.71,9.88a3,3,0,0,0,0,4.24l8.17,8.17a1,1,0,0,1,0,1.42A1,1,0,0,1,17.17,24Z"/></svg>

  `,i=`<svg class="page-choice-svg" data-id="right" data-type="${v}" data-card="${d}" xmlns="http://www.w3.org/2000/svg" id="Outline" viewBox="0 0 24 24" width="512" height="512"><path d="M7,24a1,1,0,0,1-.71-.29,1,1,0,0,1,0-1.42l8.17-8.17a3,3,0,0,0,0-4.24L6.29,1.71A1,1,0,0,1,7.71.29l8.17,8.17a5,5,0,0,1,0,7.08L7.71,23.71A1,1,0,0,1,7,24Z"/></svg>
`,a=0,l="",s=0;if(e<6)return a=1,s=e+1,r(a,s);if(e>5&&t>0&&t<4)return s=5+1,l=r(a+1,s)+i,l;if(t>3&&t+2<e)return a=t-2,s=t+3,n+r(a,s)+i;if(e<=t+2)return e==t+2?a=t-2:e-t==1?a=t-3:a=t-4,n+r(a,e+1);function r(o,u){let m="";for(let c=o;c<u;c++)t==c&&(m+=`<li>
          ...
          <button class="exercises-pages-button" data-card="${d}"
           data-number="${c}" data-type="${v}">${c}</button>
        </li>`);return m}}function V(t){if(t.target.dataset.type){f.innerHTML='<div class="loader"></div>';let e=document.querySelector(".exercises-button-active").dataset.name;e==="Body parts"&&(e="Body part"),d=t.target.dataset.type,g=1,p.classList.remove("form-is-hiden"),b.innerHTML="",S.classList.add("form-is-open"),w.innerHTML=`<span class="exer-title-slash">/</span> ${d.slice(0,1).toUpperCase()}${d.slice(1)}`,x(e,d)}t.target.dataset.id&&T(t.target.dataset.id)}async function x(t,e,n=""){try{const i=await E("exercises",t,g,k,e,n);i&&q(i.results,i.totalPages)}catch(i){console.log(i)}}function j(t){if(t.preventDefault(),h=t.target.elements.usertext.value.trim(),h.length!==0){const e=t.target.dataset.filter;t.target.dataset.status="ok",p.reset(),x(e,d,h)}}function N(t){t.target.value.length>2?L.classList.remove("reset-button-close"):L.classList.add("reset-button-close"),console.log(t.target.value)}const W=document.querySelector(".open-rating-modal");W.addEventListener("click",()=>{D(),U()});function D(){const t=document.querySelector(".container-for-modal");t&&t.remove()}function U(){const t=`
  ...
  `;document.body.insertAdjacentHTML("beforeend",t);const e=document.querySelector(".rating-modal-backdrop");document.querySelectorAll(".rating-modal-backdrop .rating-modal-close-button").forEach(i=>{i.addEventListener("click",()=>{y()})}),document.addEventListener("keydown",i=>{i.key==="Escape"&&y()}),e.addEventListener("click",i=>{i.target===e&&y()}),Z()}function y(){const t=document.querySelector(".rating-modal-backdrop");t&&t.remove()}function Z(){let t,e;const n=document.querySelector(".rating-container.rating-set.rating-modal-container");n&&(i(n),a(),l(n));function i(s){t=s.querySelector(".rating-active.rating-modal-active"),e=s.querySelector(".rating-value.rating-modal-value")}function a(s=e.innerHTML){const r=s/.05;t.style.width=`${r}%`}function l(s){const r=s.querySelectorAll(".rating-item.rating-modal-item");for(let o=0;o<r.length;o++){const u=r[o];u.addEventListener("mouseenter",function(m){i(s),a(u.value)}),u.addEventListener("mouseleave",function(m){a()}),u.addEventListener("click",function(m){i(s);const c=(m.clientX-u.getBoundingClientRect().left)/u.clientWidth,C=o+c;e.innerHTML=C.toFixed(1),a(C)})}s.addEventListener("mouseleave",function(o){})}}const _=document.querySelector(".button-modal-rating-send");_.addEventListener("click",()=>{const t=document.getElementById("email-modal"),e=document.getElementById("user-comment"),n=parseFloat(document.querySelector(".rating-value").innerText);if(t.checkValidity()&&e.checkValidity()&&n>0){const i={rate:n,email:t.value,review:e.value};axios.post(`https://energyflow.b.goit.study/api/exercises/${exerciseId}/rate`,i).then(a=>{y()}).catch(a=>{console.error(a)})}else alert("Please fill in all the required fields.")});document.querySelector(".footer-form").addEventListener("submit",function(t){t.preventDefault();const n=this.elements.email.value.trim();if(!n){alert("Field must be filled in");return}fetch("https://energyflow.b.goit.study/api/subscription",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({email:n})}).then(a=>{a.status>=200&&a.status<300&&(H.success({position:"topRight",message:"Subscription successful!"}),this.reset())}).catch(a=>{console.error(a),alert("Failed to subscribe. Please try again later.")})});
