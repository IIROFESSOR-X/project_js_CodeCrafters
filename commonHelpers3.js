import{o as M}from"./assets/quote-section-f53e3c34.js";import{a as k,i as F}from"./assets/vendor-8cce9181.js";const x=document.querySelector(".exercises-title-span"),m=document.querySelector(".exercises-list"),v=document.querySelector(".exercises-list-pages");v.addEventListener("click",q);m.addEventListener("click",N);const p=document.querySelector(".exercises-form");p.addEventListener("submit",D);p.addEventListener("input",I);const y=document.querySelector(".exercises-button-list"),L=document.querySelector(".form-reset-text-button");L.addEventListener("click",t=>{p.reset(),L.classList.add("reset-button-close")});let u=1,w=8,f="Muscles",c="",h="",$=0;const H=document.querySelectorAll(".exercises-button");[...H].map((t,e)=>{e===0&&t.classList.add("exercises-button-active"),t.addEventListener("click",S)});async function T(t,e,r,i,s,o){let a={};s&&e=="Body parts"&&(e="Body part"),s?a[e.toLowerCase().replace(" ","")]=s:a.filter=e,a.page=r,a.limit=i,o&&(a.keyword=o),document.body.clientWidth>=768&&!s&&(a.limit=12),document.body.clientWidth<=768&&s&&(a.limit=8),document.body.clientWidth>=1440&&s&&(a.limit=9);let n=new URLSearchParams(a).toString();try{const d=await k.get(`https://energyflow.b.goit.study/api/${t}?${n}`);if($=d.data.totalPages,o&&d.data.results.length==0){O();return}return d.data}catch(d){console.log(d)}}function O(){m.innerHTML=`<div class="no-results-wrapper">
...
</div>`,v.innerHTML=""}S();function S(t){if(y.classList.remove("form-is-open"),p.classList.add("form-is-hiden"),c="",x.innerHTML="",u=1,t){const e=t.target;m.innerHTML='<div class="loader"></div>',f=e.dataset.name,document.querySelector(".exercises-button-active").classList.remove("exercises-button-active"),e.classList.add("exercises-button-active")}B(f)}async function B(t){const e=await T("filters",t,u,w).then(r=>r).catch(r=>console.log(r));E(e.results,e.totalPages)}function E(t,e){if(t[0]._id){let i=t.reduce((s,{_id:o,rating:a,name:n,burnedCalories:d,time:C,bodyPart:g,target:l})=>s+=`<li class="exer-card-item">
    ...
    7.8899 3.14277 7.63525L0.930391 6.02787C0.146677 5.45846 0.549452 4.21885 1.51818 4.21885H4.25283C4.68606 4.21885 5.07001 3.93989
     5.20389 3.52786L6.04894 0.927052Z" fill="#EEA10C"/>
      ...
      rgba(16, 16, 16, 0.7) 100%
        ...
        </li>`),"");p.dataset.status="",m.classList.remove("card-row-gap-js"),m.innerHTML=i,r(t.filter)}function r(i){p.dataset.status="",v.innerHTML=A(u,e)}}function q(t){const e=t.target.dataset;if(t.target.dataset.type||e.id){if(u==t.target.dataset.number)return;if(u=t.target.dataset.number,e.id&&(u=$,e.id=="left"&&(u=1)),document.querySelector(".exercises-section").scrollIntoView({behavior:"smooth",block:"start"}),c==""){B(t.target.dataset.type);return}b(t.target.dataset.type,t.target.dataset.card,h)}}function A(t,e){t=Number.parseInt(t);let r=`<svg class="page-choice-svg" width="18" height="18" data-card="${c}" data-id="left" data-type="${f}" xmlns="http://www.w3.org/2000/svg" id="Outline" viewBox="0 0 24 24" width="512" height="512"><path d="M17.17,24a1,1,0,0,1-.71-.29L8.29,15.54a5,5,0,0,1,0-7.08L16.46.29a1,1,0,1,1,1.42,1.42L9.71,9.88a3,3,0,0,0,0,4.24l8.17,8.17a1,1,0,0,1,0,1.42A1,1,0,0,1,17.17,24Z"/></svg>
...
`,s=0,o="",a=0;if(e<6)return s=1,a=e+1,n(s,a);if(e>5&&t>0&&t<4)return a=5+1,o=n(s+1,a)+i,o;if(t>3&&t+2<e)return s=t-2,a=t+3,r+n(s,a)+i;if(e<=t+2)return e==t+2?s=t-2:e-t==1?s=t-3:s=t-4,r+n(s,e+1);function n(d,C){let g="";for(let l=d;l<C;l++)t==l&&(g+=`<li>
          ...
          <button class="exercises-pages-button" data-card="${c}"
           data-number="${l}" data-type="${f}">${l}</button>
        </li>`);return g}}function N(t){if(t.target.dataset.type){m.innerHTML='<div class="loader"></div>';let e=document.querySelector(".exercises-button-active").dataset.name;e==="Body parts"&&(e="Body part"),c=t.target.dataset.type,u=1,p.classList.remove("form-is-hiden"),v.innerHTML="",y.classList.add("form-is-open"),x.innerHTML=`<span class="exer-title-slash">/</span> ${c.slice(0,1).toUpperCase()}${c.slice(1)}`,b(e,c)}t.target.dataset.id&&M(t.target.dataset.id)}async function b(t,e,r=""){try{const i=await T("exercises",t,u,w,e,r);i&&E(i.results,i.totalPages)}catch(i){console.log(i)}}function D(t){if(t.preventDefault(),h=t.target.elements.usertext.value.trim(),h.length!==0){const e=t.target.dataset.filter;t.target.dataset.status="ok",p.reset(),b(e,c,h)}}function I(t){t.target.value.length>2?L.classList.remove("reset-button-close"):L.classList.add("reset-button-close"),console.log(t.target.value)}document.addEventListener("DOMContentLoaded",function(){document.querySelector(".scroll-up-btn").addEventListener("click",e);function e(){window.scrollTo({top:0,behavior:"smooth"})}});document.querySelector(".footer-form").addEventListener("submit",function(t){t.preventDefault();const r=this.elements.email.value.trim();if(!r){alert("Field must be filled in");return}fetch("https://energyflow.b.goit.study/api/subscription",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({email:r})}).then(s=>{s.status>=200&&s.status<300&&(F.success({position:"topRight",message:"Subscription successful!"}),this.reset())}).catch(s=>{console.error(s),alert("Failed to subscribe. Please try again later.")})});
