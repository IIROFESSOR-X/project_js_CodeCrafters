import{e as x,o as M}from"./assets/quote-section-dd88bdfc.js";import{a as q,i as L}from"./assets/vendor-8cce9181.js";const $=document.querySelector(".exercises-title-span"),u=document.querySelector(".exercises-list"),b=document.querySelector(".exercises-list-pages");b.addEventListener("click",A);u.addEventListener("click",N);const m=document.querySelector(".exercises-form");m.addEventListener("submit",D);m.addEventListener("input",U);const T=document.querySelector(".exercises-button-list"),h=document.querySelector(".form-reset-text-button");h.addEventListener("click",t=>{m.reset(),h.classList.add("reset-button-close")});let p=1,S=8,f="Muscles",c="",v="",E=0;const H=document.querySelectorAll(".exercises-button");[...H].map((t,e)=>{e===0&&t.classList.add("exercises-button-active"),t.addEventListener("click",k)});async function O(t,e,r,i,s,n){let a={};s&&e=="Body parts"&&(e="Body part"),s?a[e.toLowerCase().replace(" ","")]=s:a.filter=e,a.page=r,a.limit=i,n&&(a.keyword=n),document.body.clientWidth>=768&&!s&&(a.limit=12),document.body.clientWidth<=768&&s&&(a.limit=8),document.body.clientWidth>=1440&&s&&(a.limit=9);let o=new URLSearchParams(a).toString();try{const l=await q.get(`https://energyflow.b.goit.study/api/${t}?${o}`);if(E=l.data.totalPages,n&&l.data.results.length==0){F();return}return l.data}catch(l){console.log(l)}}function F(){u.innerHTML=`<div class="no-results-wrapper">
  <p class="no-results-message">
    ...
    opportunity to find more options that suit your needs.
        ...
        </li>`,"");u.classList.add("card-row-gap-js"),u.innerHTML=i,r()}if(!t[0]._id){let i=t.reduce((s,{filter:n,imgUrl:a,name:o})=>(m.dataset.filter=n,s+=`<li class="exercises-item" data-type="${o}" style="background-image: linear-gradient(
      0deg,
      rgba(16, 16, 16, 0.7) 0%,
      rgba(16, 16, 16, 0.7) 100%
        ...
        </li>`),"");m.dataset.status="",u.classList.remove("card-row-gap-js"),u.innerHTML=i,r(t.filter)}function r(i){m.dataset.status="",b.innerHTML=I(p,e)}}function A(t){const e=t.target.dataset;if(t.target.dataset.type||e.id){if(p==t.target.dataset.number)return;if(p=t.target.dataset.number,e.id&&(p=E,e.id=="left"&&(p=1)),document.querySelector(".exercises-section").scrollIntoView({behavior:"smooth",block:"start"}),c==""){B(t.target.dataset.type);return}u.innerHTML='<div class="loder-container-card" ><div class="loader"></div></div>',w(t.target.dataset.type,t.target.dataset.card,v)}}function I(t,e){t=Number.parseInt(t);let r=`<svg  class="page-choice-svg"  width="18"  height="18"  data-card="${c}"  data-id="left"  data-type="${f}"
