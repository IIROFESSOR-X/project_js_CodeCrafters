import{e as w,o as O}from"./assets/toggle-btn-theme-f1c892c3.js";import{a as q,i as x}from"./assets/vendor-8cce9181.js";const B="/project_js_CodeCrafters/assets/icons-58ead758.svg";function A(){document.body.insertAdjacentHTML("beforeend",`<button class="scroll-top-btn">
    ...
    opportunity to find more options that suit your needs.
      ...
      rgba(16, 16, 16, 0.7) 100%
          ...
          <button class="exercises-pages-button" data-card="${d}"
           data-number="${c}" data-type="${g}">${c}</button>
        </li>`);return f}}function F(t){if(t.target.dataset.type){l.innerHTML='<div class="loder-container-card" ><div class="loader"></div></div>';let e=document.querySelector(".exercises-button-active").dataset.name;e==="Body parts"&&(e="Body part"),d=t.target.dataset.type,p=1,m.classList.remove("form-is-hiden"),b.innerHTML="",S.classList.add("form-is-open"),$.innerHTML=`<span class="exer-title-slash">/</span> ${d.slice(0,1).toUpperCase()}${d.slice(1)}`,L(e,d)}t.target.dataset.id&&O(t.target.dataset.id)}async function L(t,e,r=""){try{const i=await H("exercises",t,p,T,e,r);i&&k(i.results,i.totalPages)}catch(i){console.log(i)}}function U(t){if(t.preventDefault(),h=t.target.elements.usertext.value.trim(),h.length!==0){const e=t.target.dataset.filter;t.target.dataset.status="ok",m.reset(),v.classList.add("reset-button-close"),L(e,d,h)}}function D(t){t.target.value.length>2?v.classList.remove("reset-button-close"):v.classList.add("reset-button-close")}A();document.querySelector(".footer-form").addEventListener("submit",function(t){t.preventDefault();const r=this.elements.email.value.trim();if(!/^[\w-]+(\.[\w-]+)*@([\w-]+\.)+[a-zA-Z]{2,7}$/.test(r)){x.error({position:"topRight",message:"please enter a valid email!"});return}fetch("https://energyflow.b.goit.study/api/subscription",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({email:r})}).then(s=>{s.status>=200&&s.status<300&&(x.success({position:"topRight",message:"Subscription successful!"}),this.reset())}).catch(s=>{console.error(s),x.error({position:"topRight",message:"Sorry! You are already subscribed"}),this.reset()})});
