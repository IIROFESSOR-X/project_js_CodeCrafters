import{o as f}from"./assets/quote-section-9eaabcdc.js";import"./assets/vendor-8cce9181.js";let d,t;const s={favoritesList:document.querySelector(".favorites-list"),gymInfo:document.querySelector(".gym-info")};window.addEventListener("load",L);s.favoritesList.addEventListener("click",g);s.favoritesList.addEventListener("click",u);const o=a=>{const e=a.reduce((i,{_id:r,name:n,burnedCalories:c,time:l,bodyPart:C,target:v})=>i+`<li class="exer-card-item">
  <div class="exer-card-background">
    <div class="card-workout-wrapper">
      <p class="exer-workout-text">WORKOUT</p>
      <button class="card-btn-delete" data-action="delete" data-id="${r}" aria-label="remove an exercise from your favorites">
        <svg class="card-btn-delete-svg" width="16" height="16" viewBox="0 0 32 32" data-action="delete" data-id="${r}">
<path data-action="delete" stroke-linejoin="round" stroke-linecap="round" stroke-miterlimit="4" stroke-width="2.6" d="M21.333 8v-1.067c0-1.493 0-2.24-0.291-2.811-0.256-0.502-0.664-0.91-1.165-1.165-0.57-0.291-1.317-0.291-2.811-0.291h-2.133c-1.493 0-2.24 0-2.811 0.291-0.502 0.256-0.91 0.664-1.165 1.165-0.291 0.57-0.291 1.317-0.291 2.811v1.067M13.333 15.333v6.667M18.667 15.333v6.667M4 8h24M25.333 8v14.933c0 2.24 0 3.36-0.436 4.216-0.384 0.753-0.995 1.365-1.748 1.748-0.856 0.436-1.976 0.436-4.216 0.436h-5.867c-2.24 0-3.36 0-4.216-0.436-0.753-0.384-1.365-0.995-1.748-1.748-0.436-0.856-0.436-1.976-0.436-4.216v-14.933"></path>        </svg>
      ...
      <button class="card-start-button" data-action="start" data-id="${r}">
         Start
        <svg class="card-arrow-svg" data-action="start" data-id="${r}"  width="16" height="16" viewBox="0 0 16 16" fill="none">
    ...
    <p class="card-target-text">
      Target:
      <span class="card-burned-calories-span">${v}</span>
    </p>
  </div>
</li>`,"");s.favoritesList.innerHTML=e};function p(){d=localStorage.getItem("favorites");try{t=JSON.parse(d)}catch(a){console.log(a),t=[]}}function g(a){if(a.target.dataset.action!=="delete")return;const e=a.target.dataset.id,i=t.find(({_id:n})=>n===e),r=t.indexOf(i);t.splice(r,1),localStorage.setItem("favorites",JSON.stringify(t)),t.length||s.gymInfo.classList.remove("is-hidden"),o(t)}function u(a){if(a.target.dataset.action!=="start")return;const e=a.target.dataset.id;e&&f(e)}function L(){p(),t.length?(s.gymInfo.classList.add("is-hidden"),o(t)):s.gymInfo.classList.remove("is-hidden")}
