...
</li>`,"");u.favoritesList.innerHTML=n}};function y(){$=localStorage.getItem("favorites");try{c=JSON.parse($),c||(c=[])}catch(t){console.log(t),c=[]}}function B(t){if(t.target.dataset.action!=="delete")return;y();const n=t.target.dataset.id,o=c.filter(({_id:a})=>a!==n);localStorage.setItem("favorites",JSON.stringify(o)),y(),c.length||u.gymInfo.classList.remove("is-hidden"),x.innerHTML="",S(c)}function T(t){if(t.target.dataset.action!=="start")return;const n=t.target.dataset.id;n&&I(n)}const x=document.querySelector(".container-for-pagination");function H(){y(),c.length&&u.gymInfo?(u.gymInfo.classList.add("is-hidden"),S(c),x.addEventListener("click",W),u.favoritesList.addEventListener("click",B),u.favoritesList.addEventListener("click",T)):u.gymInfo&&(u.gymInfo.classList.remove("is-hidden"),u.favoritesList.removeEventListener("click",B),u.favoritesList.removeEventListener("click",T))}const J=document.querySelector(".exercises-section-fv");function W(t){if(t.target.dataset.number){let n=t.target.dataset.number;if(n===v)return;v=n}t.target.dataset&&(t.target.dataset.id=="left"?v=1:v=Math.ceil(c.length/8)),y(),S(c)}const V=()=>{let t=c.length;if(t<1)return;let n=v*8;return x.innerHTML=Q(v,Math.ceil(t/8)),J.scrollIntoView({behavior:"smooth",block:"start"}),c.slice(n-8,n)};function Q(t,n){t=Number.parseInt(t);let o=`<li data-id="left"><svg  class="page-choice-svg"  width="18"  height="18"    data-id="left"  
