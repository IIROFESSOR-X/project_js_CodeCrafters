...
<path data-action="delete" stroke-linejoin="round" stroke-linecap="round" stroke-miterlimit="4" stroke-width="2.6" d="M21.333 8v-1.067c0-1.493 0-2.24-0.291-2.811-0.256-0.502-0.664-0.91-1.165-1.165-0.57-0.291-1.317-0.291-2.811-0.291h-2.133c-1.493 0-2.24 0-2.811 0.291-0.502 0.256-0.91 0.664-1.165 1.165-0.291 0.57-0.291 1.317-0.291 2.811v1.067M13.333 15.333v6.667M18.667 15.333v6.667M4 8h24M25.333 8v14.933c0 2.24 0 3.36-0.436 4.216-0.384 0.753-0.995 1.365-1.748 1.748-0.856 0.436-1.976 0.436-4.216 0.436h-5.867c-2.24 0-3.36 0-4.216-0.436-0.753-0.384-1.365-0.995-1.748-1.748-0.436-0.856-0.436-1.976-0.436-4.216v-14.933"></path>        </svg>
...
</li>`,"");c.favoritesList.innerHTML=n}};function R(){M=localStorage.getItem("favorites");try{d=JSON.parse(M)}catch(t){console.log(t),d=[]}}function E(t){if(t.target.dataset.action!=="delete")return;const n=t.target.dataset.id,a=d.find(({_id:e})=>e===n),o=d.indexOf(a);d.splice(o,1),localStorage.setItem("favorites",JSON.stringify(d)),d.length||c.gymInfo.classList.remove("is-hidden"),C(d)}function q(t){if(t.target.dataset.action!=="start")return;const n=t.target.dataset.id;n&&S(n)}function T(){R(),d.length&&c.gymInfo?(c.gymInfo.classList.add("is-hidden"),C(d),c.favoritesList.addEventListener("click",E),c.favoritesList.addEventListener("click",q)):c.gymInfo&&(c.gymInfo.classList.remove("is-hidden"),c.favoritesList.removeEventListener("click",E),c.favoritesList.removeEventListener("click",q))}let k="",O,b;function j(t){k=t,document.querySelector(".modal-button-rating").addEventListener("click",n=>{f(),A()})}function A(){const t=`
  ...
  `,n=document.querySelector(".container-for-modal");n.innerHTML=t;const a=document.querySelector(".rating-modal-backdrop");document.querySelectorAll(".rating-modal-backdrop .rating-modal-close-button").forEach(e=>{e.addEventListener("click",()=>{v()})}),document.addEventListener("keydown",e=>{e.key==="Escape"&&v()}),a.addEventListener("click",e=>{e.target===a&&v()}),N(),P()}function v(){const t=document.querySelector(".rating-modal-backdrop");t&&(t.remove(),S(k))}function N(){const t=document.querySelector(".rating-container.rating-set.rating-modal-container");t&&(n(t),a(),o(t));function n(e){O=e.querySelector(".rating-active.rating-modal-active"),b=e.querySelector(".rating-value.rating-modal-value")}function a(e=b.innerHTML){const i=e/.05;O.style.width=`${i}%`}function o(e){const i=e.querySelectorAll(".rating-item.rating-modal-item");for(let r=0;r<i.length;r++){const s=i[r];s.addEventListener("mouseenter",function(m){n(e),a(s.value)}),s.addEventListener("mouseleave",function(m){a()}),s.addEventListener("click",function(m){n(e);const l=(m.clientX-s.getBoundingClientRect().left)/s.clientWidth,g=Math.min(5,r+l);b.innerHTML=g.toFixed(1),a(g)})}e.addEventListener("mouseleave",function(r){e.classList.remove("interacting")})}}function P(){document.querySelector(".button-modal-rating-send").addEventListener("click",async n=>{n.preventDefault();const a=document.getElementById("email-modal"),o=document.getElementById("user-comment"),e=parseFloat(document.querySelector(".rating-value").innerText);if(a.checkValidity()&&o.checkValidity()&&e>=0){let i={rate:e,email:a.value,review:o.value};try{await L.patch(`https://energyflow.b.goit.study/api/exercises/${k}/rating/`,i),v()}catch(r){console.error(r.response.data),w.error({title:"Error",message:"Failed to submit the rating. Please try again later."})}}else w.warning({title:"Warning",message:"Please fill in all the required fields."})})}const p=document.querySelector(".container-for-modal"),S=t=>{try{L({method:"get",url:`https://energyflow.b.goit.study/api/exercises/${t}`}).then(a=>{D(a.data),J(a.data.rating),H(a.data)}).catch(a=>{console.log(a)}).finally(()=>{p.removeEventListener("click",S),document.addEventListener("click",x),document.addEventListener("keydown",I)})}catch(n){console.error(n)}};function D(t={}){const n=`
        <div class="overlay">
        ...
        </div>`;p.innerHTML=n,j(t._id)}function J(t){const n=t/5*100,a=`${Math.round(n/10)*10}%`;document.querySelector(".rating .stars-inner").style.width=a}function x(t){(t.target.classList.contains("modal-close-button")||t.target.classList.contains("overlay"))&&f()}function I(t){t.key==="Escape"&&f()}function f(){p.innerHTML="",document.removeEventListener("keydown",I),document.removeEventListener("click",x)}const y=()=>{let t=JSON.parse(localStorage.getItem("favorites"));C(t)};function H(t){const n=localStorage.getItem("favorites")||"[]",a=t;let o;try{o=JSON.parse(n)}catch(l){console.log(l),o=[]}Array.isArray(o)||(o=[]);const e=p.querySelector(".modal-button-favorites"),i=e.querySelector(".btn-text"),r=()=>{const l=o.filter(g=>g._id!==a._id);localStorage.setItem("favorites",JSON.stringify(l)),i.textContent="Add to favorites",y(),e.removeEventListener("click",r),e.addEventListener("click",s)},s=()=>{let l=!1;l||(o.find(F=>F._id===a._id)?(localStorage.setItem("favorites",JSON.stringify(o)),i.textContent="Remove from",y(),e.removeEventListener("click",s),e.addEventListener("click",r)):(o.push(a),localStorage.setItem("favorites",JSON.stringify(o)),i.textContent="Remove from",y(),l=!0,e.removeEventListener("click",s),e.addEventListener("click",r)))};e.removeEventListener("click",r),e.removeEventListener("click",s),o.find(l=>l._id===a._id)?(e.addEventListener("click",r),i.textContent="Remove from"):(e.addEventListener("click",s),i.textContent="Add to favorites")}const Q=document.querySelector(".quote-text"),V=document.querySelector(".quote-author"),W=async()=>{try{return(await L.get("https://energyflow.b.goit.study/api/quote")).data}catch(t){console.log(t.message)}},_=(t,n)=>{const a={quote:t,author:n,date:new Date().toDateString()};localStorage.setItem("quote-data",JSON.stringify(a))},K=()=>{const t=localStorage.getItem("quote-data");if(t){const{date:n,quote:a,author:o}=JSON.parse(t);if(n===new Date().toDateString()){$(a,o);return}}U()},$=(t,n)=>{Q.textContent=t,V.textContent=n},U=async()=>{const t=await W();$(t.quote,t.author),_(t.quote,t.author)};K();export{S as o};
//# sourceMappingURL=quote-section-1e5a2954.js.map
