import{a as w,i as v}from"./vendor-8cce9181.js";(function(){const n=document.createElement("link").relList;if(n&&n.supports&&n.supports("modulepreload"))return;for(const e of document.querySelectorAll('link[rel="modulepreload"]'))a(e);new MutationObserver(e=>{for(const i of e)if(i.type==="childList")for(const s of i.addedNodes)s.tagName==="LINK"&&s.rel==="modulepreload"&&a(s)}).observe(document,{childList:!0,subtree:!0});function o(e){const i={};return e.integrity&&(i.integrity=e.integrity),e.referrerPolicy&&(i.referrerPolicy=e.referrerPolicy),e.crossOrigin==="use-credentials"?i.credentials="include":e.crossOrigin==="anonymous"?i.credentials="omit":i.credentials="same-origin",i}function a(e){if(e.ep)return;e.ep=!0;const i=o(e);fetch(e.href,i)}})();const u={mobileMenu:document.querySelector("[data-mobile-menu]"),mobileMenuBtnClose:document.querySelector("[data-mobile-menu-close]"),mobileMenuBtnOpen:document.querySelector("[data-modal-open]")};u.mobileMenu.classList.contains("is-hidden")&&u.mobileMenuBtnOpen.addEventListener("click",k);function k(){u.mobileMenu.classList.remove("is-hidden"),u.mobileMenuBtnClose.addEventListener("click",I),u.mobileMenuBtnOpen.removeEventListener("click",k)}function I(){u.mobileMenu.classList.add("is-hidden"),u.mobileMenuBtnClose.removeEventListener("click",I),u.mobileMenuBtnOpen.addEventListener("click",k)}const y="/project_js_CodeCrafters/assets/symbol-defs-dfb5c4f7.svg";function A(){document.body.insertAdjacentHTML("beforeend",`<button class="scroll-top-btn">
...
</li>`,"");c.favoritesList.innerHTML=n}};function S(){O=localStorage.getItem("favorites");try{d=JSON.parse(O),d||(d=[])}catch(t){console.log(t),d=[]}}function x(t){if(t.target.dataset.action!=="delete")return;S();const n=t.target.dataset.id,o=d.filter(({_id:a})=>a!==n);localStorage.setItem("favorites",JSON.stringify(o)),S(),d.length||c.gymInfo.classList.remove("is-hidden"),E(d)}function B(t){if(t.target.dataset.action!=="start")return;const n=t.target.dataset.id;n&&M(n)}function N(){S(),d.length&&c.gymInfo?(c.gymInfo.classList.add("is-hidden"),E(d),c.favoritesList.addEventListener("click",x),c.favoritesList.addEventListener("click",B)):c.gymInfo&&(c.gymInfo.classList.remove("is-hidden"),c.favoritesList.removeEventListener("click",x),c.favoritesList.removeEventListener("click",B))}A();let q="",C,h;function P(t){q=t,document.querySelector(".modal-button-rating").addEventListener("click",n=>{p(),F()})}function F(){const t=`
  ...
  `,n=document.querySelector(".container-for-modal");n.innerHTML=t;const o=document.querySelector(".rating-modal-backdrop");document.querySelectorAll(".rating-modal-backdrop .rating-modal-close-button").forEach(e=>{e.addEventListener("click",()=>{f()})}),document.addEventListener("keydown",e=>{e.key==="Escape"&&f()}),o.addEventListener("click",e=>{e.target===o&&f()}),D(),H()}function f(){const t=document.querySelector(".rating-modal-backdrop");t&&(t.remove(),M(q))}function D(){const t=document.querySelector(".rating-container.rating-set.rating-modal-container");t&&(n(t),o(),a(t));function n(e){C=e.querySelector(".rating-active.rating-modal-active"),h=e.querySelector(".rating-value.rating-modal-value")}function o(e=h.innerHTML){const i=e/.05;C.style.width=`${i}%`}function a(e){const i=e.querySelectorAll(".rating-item.rating-modal-item");for(let s=0;s<i.length;s++){const r=i[s];r.addEventListener("mouseenter",function(m){n(e),o(r.value)}),r.addEventListener("mouseleave",function(m){o()}),r.addEventListener("click",function(m){n(e);const l=(m.clientX-r.getBoundingClientRect().left)/r.clientWidth,g=Math.min(5,s+l);h.innerHTML=g.toFixed(1),o(g)})}e.addEventListener("mouseleave",function(s){e.classList.remove("interacting")})}}function H(){document.querySelector(".button-modal-rating-send").addEventListener("click",async n=>{n.preventDefault();const o=document.getElementById("email-modal"),a=document.getElementById("user-comment"),e=parseFloat(document.querySelector(".rating-value").innerText);if(o.checkValidity()&&a.checkValidity()&&e>=0){let i={rate:e,email:o.value,review:a.value};try{await w.patch(`https://energyflow.b.goit.study/api/exercises/${q}/rating/`,i),f()}catch(s){console.error(s.response.data),v.error({title:"Error",message:"Failed to submit the rating. Please try again later."})}}else v.warning({title:"Warning",message:"Please fill in all the required fields."})})}const b=document.querySelector(".container-for-modal"),M=t=>{try{w({method:"get",url:`https://energyflow.b.goit.study/api/exercises/${t}`}).then(o=>{J(o.data),Q(o.data.rating),V(o.data)}).catch(o=>{console.log(o)}).finally(()=>{b.removeEventListener("click",M),document.addEventListener("click",$),document.addEventListener("keydown",T)})}catch(n){console.error(n)}};function J(t={}){const n=`
        <div class="overlay">
